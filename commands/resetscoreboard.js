...
const { buildScoreboardEmbed, hasPermission } = require('../utils');

module.exports = {
  data: new SlashCommandBuilder()
    .setName('resetscoreboard')
    .setDescription('Reset all scores on a scoreboard to zero')
  ...
  async execute(interaction) {
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`reset_confirm_${sb.id}`).setLabel('Yes, reset it').setStyle(ButtonStyle.Danger),
      new ButtonBuilder().setCustomId('reset_cancel').setLabel('Cancel').setStyle(ButtonStyle.Secondary)
    );

    await interaction.reply({
      content: `⚠️ Are you sure you want to reset **${sb.name}**? This cannot be undone.`,
      components: [row],
      ephemeral: true
    });
  }
